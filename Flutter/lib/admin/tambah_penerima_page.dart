...
class _TambahPenerimaPageState extends State<TambahPenerimaPage> {
  ...
  Future<void> _simpanData() async {
    ...
    if (tanggalMulai == null) {
      _showSnackBar('Tanggal mulai harus dipilih', Colors.red);
      return;
    }
    if (tanggalSelesai == null) {
      _showSnackBar('Tanggal selesai harus dipilih', Colors.red);
      return;
    }
    ...
    final response = await ApiService.createProgramDonasi(
      ...
      emisiDonasi: terkumpulController.text.isEmpty 
          ? 0 
          : double.parse(terkumpulController.text.replaceAll(RegExp(r'[^0-9]'), '')),
    );
    ...
    if (mounted) {
      if (response['status'] == true) {
        _showSnackBar('Program donasi berhasil ditambahkan', Colors.green);
        Navigator.pop(context, true);
      } else {
        _showSnackBar(
          response['message'] ?? 'Gagal menambahkan program',
          Colors.red,
        );
      }
    }
